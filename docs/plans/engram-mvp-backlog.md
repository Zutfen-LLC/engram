...
- **Why it matters:** Automated memory capture is the end-state product loop — but it is post-MVP because explicit MCP-driven dogfooding works without it, the library itself is fully written (~1,200 lines), and the blocking dependency (the upstream hook) is outside this repo's control.
- **Affected files/components:** `adapters/engram-hooks/` (new `tests/`), Hermes profile config (external), CI.
- **Verification requirement:** Unit suite in CI; a recorded Hermes session showing extraction → classification → proposed write → (BL-004) auto-promotion → startup recall containing the memory; shim behavior verified on a Hermes version without the hook.
- **Dependencies:** BL-005, BL-008, BL-009; externally, Hermes PR #59898 or shim validation in lieu.
