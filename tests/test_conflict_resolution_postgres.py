...
Concurrency is intentionally outside this module (P0-FIX-003B).
"""
...
async def proof(monkeypatch: pytest.MonkeyPatch) -> AsyncIterator[dict[str, Any]]:
    ...
    async with owner.begin() as conn:
        await conn.execute(text("INSERT INTO tenants (id,name,slug) VALUES (:id,:n,:n)"), {"id": tenant, "n": f"conflict-proof-{tag}"})
        await conn.execute(text("INSERT INTO tenant_config (tenant_id,config_version,active) VALUES (:id,'proof',true)"), {"id": tenant})
        await conn.execute(text("INSERT INTO workspaces (id,tenant_id,name,slug) VALUES (:id,:tid,:n,:n)"), {"id": workspace, "tid": tenant, "n": f"restricted-{tag}"})
        for name, (pid, ptype, scopes, internal_key) in actors.items():
            await conn.execute(text("INSERT INTO principals (id,tenant_id,name,type,internal_key) VALUES (:id,:tid,:n,:t,:ik)"), {"id": pid, "tid": tenant, "n": f"{name}-{tag}", "t": ptype, "ik": internal_key})
            if scopes:
                token = generate_api_key()
                parsed = parse_api_key(token)
                assert parsed.key_id
                keys[name] = token
                await conn.execute(text("INSERT INTO api_keys (id,tenant_id,principal_id,key_id,secret_digest,digest_algorithm,scopes,label) VALUES (:id,:tid,:pid,:kid,:digest,:algorithm,:scopes,:label)"), {"id": uuid.uuid4(), "tid": tenant, "pid": pid, "kid": parsed.key_id, "digest": digest_api_key_secret(parsed.secret), "algorithm": DIGEST_ALGORITHM, "scopes": scopes, "label": f"conflict-proof-{tag}-{name}"})

    async def pair(*, target_visibility: str = "tenant", counterpart_visibility: str = "tenant", target_workspace: uuid.UUID | None = None, counterpart_workspace: uuid.UUID | None = None, reciprocal: bool = True) -> tuple[uuid.UUID, uuid.UUID]:
        target, counterpart = uuid.uuid4(), uuid.uuid4()
        async with owner.begin() as conn:
            for iid, label, visibility, wid in ((target, "target", target_visibility, target_workspace), (counterpart, "counterpart", counterpart_visibility, counterpart_workspace)):
                await conn.execute(text("INSERT INTO memory_items (id,tenant_id,workspace_id,principal_id,content,content_hash,kind,visibility,review_status,memory_confidence,source_trust,importance,source_type) VALUES (:id,:tid,:wid,:pid,:content,:hash,'fact',:visibility,'active',.81,.73,.64,'manual')"), {"id": iid, "tid": tenant, "wid": wid, "pid": actors["author"][0], "content": f"{tag}:{label}:{iid}", "hash": f"sha256:{iid.hex}", "visibility": visibility})
            await conn.execute(text("UPDATE memory_items SET conflicts_with_item_id=:cp, conflict_type='contradiction', conflict_resolution_status='unresolved' WHERE id=:id"), {"id": target, "cp": counterpart})
            if reciprocal:
                await conn.execute(text("UPDATE memory_items SET conflicts_with_item_id=:cp, conflict_type='contradiction', conflict_resolution_status='unresolved' WHERE id=:id"), {"id": counterpart, "cp": target})
        return target, counterpart

    async def state(item_id: uuid.UUID) -> dict[str, Any]:
        async with owner.connect() as conn:
            row = (await conn.execute(text("SELECT * FROM memory_items WHERE id=:id"), {"id": item_id})).mappings().one()
            events = (await conn.execute(text("SELECT * FROM item_events WHERE item_id=:id AND event_type='conflict_resolution' ORDER BY created_at,id"), {"id": item_id})).mappings().all()
    ...
    try:
        ...
        await client.aclose()
        async with owner.begin() as conn:
            await conn.execute(text("DROP TRIGGER IF EXISTS conflict_proof_fail_event ON item_events"))
            await conn.execute(text("DROP FUNCTION IF EXISTS conflict_proof_fail_event()"))
            await conn.execute(text("DROP TRIGGER IF EXISTS conflict_proof_fail_update ON memory_items"))
            await conn.execute(text("DROP FUNCTION IF EXISTS conflict_proof_fail_update()"))
            await conn.execute(
                text("DELETE FROM item_events WHERE actor_principal_id IN (SELECT id FROM principals WHERE tenant_id=:id)"),
                {"id": tenant},
            )
...
async def _resolve(p: dict[str, Any], actor: str, item_id: uuid.UUID, resolution: str = "accepted", **body: Any) -> Any:
    return await p["client"].post(f"/v1/items/{item_id}/resolve-conflict", json={"resolution": resolution, **body}, headers=_headers(p, actor))
...
async def test_app_role_force_rls_and_principal_type_scope_matrix(proof: dict[str, Any]) -> None:
    async with proof["app"].connect() as conn:
        ...
        forced = dict((await conn.execute(text("SELECT relname,relforcerowsecurity FROM pg_class WHERE relname IN ('memory_items','item_events')"))).all())
    assert role == (False, False)
    assert forced == {"memory_items": True, "item_events": True}
    expected = {"user_review": 200, "admin_review": 200, "agent_review": 403, "agent_admin": 403, "system_review": 403, "system_admin": 403, "user_no_review": 403, "admin_no_review": 403}
    for actor, status in expected.items():
        target, counterpart = await proof["pair"]()
        before_target, before_counterpart = await proof["state"](target), await proof["state"](counterpart)
        response = await _resolve(proof, actor, target)
        assert response.status_code == status, (actor, response.text)
        after_target, after_counterpart = await proof["state"](target), await proof["state"](counterpart)
        if status == 200:
...
async def test_eligibility_precedes_actor_class_and_denials_are_inert(proof: dict[str, Any], edge: str) -> None:
    kwargs: dict[str, Any] = {}
    ...
    assert (await proof["state"](eligible_target), await proof["state"](eligible_counterpart)) == before


@pytest.mark.parametrize(("actor", "resolution"), [("user_review", "accepted"), ("admin_review", "rejected"), ("user_review", "merged")])
async def test_success_attribution_event_and_metadata_only(proof: dict[str, Any], actor: str, resolution: str) -> None:
    target, counterpart = await proof["pair"]()
    before_target, before_counterpart = await proof["state"](target), await proof["state"](counterpart)
    ...
    changed = {key for key in before_target["item"] if before_target["item"][key] != after_target["item"][key]}
    ...
    if resolution == "merged":
        async with proof["owner"].connect() as conn:
            count = await conn.scalar(text("SELECT count(*) FROM memory_items WHERE tenant_id=:tid"), {"tid": proof["tenant"]})
        assert count == 2


async def test_delegation_records_represented_principal_separately_and_internal_fails_closed(proof: dict[str, Any]) -> None:
    target, _ = await proof["pair"]()
    response = await _resolve(proof, "admin_review", target, on_behalf_of_principal_id=str(proof["actors"]["delegate"]))
    assert response.status_code == 200
    ...
    assert json.loads(state["events"][0]["reason"])["on_behalf_of_principal_id"] == str(proof["actors"]["delegate"])
    ...
    denied = await _resolve(proof, "admin_review", failed_target, on_behalf_of_principal_id=str(proof["actors"]["automation"]))
    assert denied.status_code == 404
    assert (await proof["state"](failed_target), await proof["state"](failed_counterpart)) == before


async def test_retries_preserve_original_attribution_reason_and_legacy_unknown(proof: dict[str, Any]) -> None:
    target, _ = await proof["pair"]()
    first = await _resolve(proof, "admin_review", target, reason="original", on_behalf_of_principal_id=str(proof["actors"]["delegate"]))
    ...
    async with proof["owner"].begin() as conn:
        await conn.execute(text("UPDATE memory_items SET conflict_resolution_status='merged', conflict_resolved_at=now(), conflict_resolved_by=NULL WHERE id=:id"), {"id": legacy})
...
async def test_postgres_failure_rolls_back_then_request_recovers(proof: dict[str, Any], failure: str) -> None:
    target, _ = await proof["pair"]()
    async with proof["owner"].begin() as conn:
        if failure == "event":
            await conn.execute(text("CREATE FUNCTION conflict_proof_fail_event() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'forced conflict event failure'; END $$"))
            await conn.execute(text(f"CREATE TRIGGER conflict_proof_fail_event BEFORE INSERT ON item_events FOR EACH ROW WHEN (NEW.event_type='conflict_resolution' AND NEW.item_id='{target}') EXECUTE FUNCTION conflict_proof_fail_event()"))
        else:
            await conn.execute(text("CREATE FUNCTION conflict_proof_fail_update() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'forced conflict update failure'; END $$"))
            await conn.execute(text(f"CREATE TRIGGER conflict_proof_fail_update BEFORE UPDATE ON memory_items FOR EACH ROW WHEN (OLD.id='{target}' AND OLD.conflict_resolution_status='unresolved') EXECUTE FUNCTION conflict_proof_fail_update()"))
    ...
    async with proof["owner"].begin() as conn:
        await conn.execute(text(f"DROP TRIGGER conflict_proof_fail_{failure} ON {'item_events' if failure == 'event' else 'memory_items'}"))
