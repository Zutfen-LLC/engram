"""Contracts for the #162C holdout Reviewer-A freeze and Reviewer-B handoff."""
...
def _ledger() -> dict[str, object]:
    ...
    return {
        "ledger_version": "eng-calibration-001c-holdout-reviewer-a-ledger-v1",
        "policy_blind": True,
...
def _expand(ledger: dict[str, Any] | None = None) -> dict[str, Any]:
    from evals.admission.holdout.adjudication import expand_holdout_reviewer_a

    return expand_holdout_reviewer_a(
        _packet(),
...
def test_expansion_is_deterministic_and_preserves_unknown_provenance() -> None:
    ...
    assert label["dataset_id"] == "eng-calibration-001c"
...
@pytest.mark.parametrize("mutation", ["missing", "duplicate", "unknown", "summary_drift"])
def test_ledger_fail_closed(mutation: str) -> None:
    ledger = _ledger()
    decisions = ledger["decisions"]
    assert isinstance(decisions, list)
    if mutation == "missing":
        decisions.pop()
        ledger["summary"] = _summary(decisions)
...
def test_high_consequence_freeze_is_schema_valid_but_reviewer_b_pending() -> None:
    ...
    low = frozen["records"][0]["label"]
    assert low["review_stage"] == "complete"


def test_reviewer_b_flag_drift_fails_closed() -> None:
    ledger = _ledger()
    decisions = ledger["decisions"]
    assert isinstance(decisions, list)
    decisions[1]["reviewer_b_required"] = False
    with pytest.raises(ValueError, match="reviewer_b_flag_drift"):
        _expand(ledger)


def test_policy_reveal_fails_closed() -> None:
    ledger = _ledger()
    decisions = ledger["decisions"]
    assert isinstance(decisions, list)
    decisions[0]["policy_reveal_performed"] = True
    with pytest.raises(ValueError):
        _expand(ledger)


def test_reviewer_b_packet_derives_only_high_and_leaks_no_reviewer_a_or_policy() -> None:
    from evals.admission.reviewer_adjudication import reviewer_b_queue

    frozen = _expand()
    packet, state = reviewer_b_queue(frozen, _packet())
    assert [case["review_case_id"] for case in packet["cases"]] == [
    ...
    assert state["review_state_schema_version"] == "engram-reviewer-b-state-v1"
