...
class TestBlindPackets:
    ...
    def test_ingestion_fails_closed(self, tmp_path: Path):
        ...
        with pytest.raises((ValueError, ValidationError)):
            ingest_reviewer_labels(
                packet=packet, labels=labels[:-1], dataset_id="ds", dataset_version="v1"
            )
        # wrong dataset identity fails
        with pytest.raises((ValueError, ValidationError)):
            ingest_reviewer_labels(
                packet=packet, labels=labels, dataset_id="other", dataset_version="v1"
            )
...
class TestFitting:
    ...
    def test_fit_deterministic_and_drops_thin_strata(self):
        ...
        p1 = fit_profiles(dev, identity=identity, contract=contract)
        p2 = fit_profiles(dev, identity=identity, contract=contract)
        ...
        assert fit_profiles(thin, identity=identity, contract=contract) == []

    def test_holdout_never_fitted(self):
        identity = _identity()
        contract = _contract(identity)
        dev = self._observations(split_of="dev")
        holdout = self._observations(split_of="holdout")
        profiles = fit_profiles(dev + holdout, identity=identity, contract=contract)
        # fitting on dev+holdout must equal fitting on dev alone
        assert [p.model_dump() for p in profiles] == [
            p.model_dump() for p in fit_profiles(dev, identity=identity, contract=contract)
        ]
        metrics = evaluate_holdout(dev + holdout, profiles=profiles)
        assert metrics and all(m.n > 0 for m in metrics)

    def test_loader_reproduces_calibration_state(self, tmp_path: Path):
        identity = _identity()
        contract = _contract(identity)
        profiles = fit_profiles(self._observations(), identity=identity, contract=contract)
