...
class ServiceClient(Base):
    """A non-principal control-plane caller, never usable as a tenant actor."""
...
class ServiceClientCredential(Base):
    ...
    service_client_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("service_clients.id", ondelete="RESTRICT"), nullable=False
    )
...
class TenantProvisioningBinding(Base):
    __tablename__ = "tenant_provisioning_bindings"
...
class PrincipalProvisioningBinding(Base):
    __tablename__ = "principal_provisioning_bindings"
...
class MemoryItem(Base):
    ...
    explicit_priority: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class ItemEvent(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            name="fk_item_events_memory_profile_revision",
            ondelete="NO ACTION",
            deferrable=True,
            initially="DEFERRED",
        ),
...
class RecallLog(Base):
    ...
    # context. Current enforcing paths explicitly supply memory-context-v2;
    # historical 002B rows remain memory-context-v1.
    memory_context_version: Mapped[str] = mapped_column(
        Text,
        default="legacy-unprofiled-v0",
        server_default=text("'legacy-unprofiled-v0'"),
...
class CandidateIngest(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["tenant_id", "api_key_id"],
            ["api_keys.tenant_id", "api_keys.id"],
            name="fk_candidate_ingests_api_key",
            ondelete="SET NULL",
            deferrable=True,
            initially="DEFERRED",
        ),
        ForeignKeyConstraint(
            ...
            name="fk_candidate_ingests_memory_profile_revision",
            ondelete="NO ACTION",
            deferrable=True,
            initially="DEFERRED",
        ),
